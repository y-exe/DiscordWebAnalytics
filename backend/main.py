...
if not re.fullmatch(r"pbkdf2-sha256\$600000\$[A-Za-z0-9_-]+\$[A-Za-z0-9_-]+", ADMIN_PASSWORD_HASH):
    sys.exit("ERROR: ADMIN_PASSWORD_HASH must be a PBKDF2-SHA256 hash generated by scripts/generate_admin_password_hash.py.")
...
def verify_admin_password(password: str) -> bool:
    if not isinstance(password, str) or not 0 < len(password) <= 256:
        return False
    try:
        _algorithm, iterations_raw, salt_raw, hash_raw = ADMIN_PASSWORD_HASH.split("$", 3)
        if iterations_raw != "600000":
            return False
        padding = "=" * (-len(salt_raw) % 4)
        salt = base64.urlsafe_b64decode(salt_raw + padding)
        expected = base64.urlsafe_b64decode(hash_raw + "=" * (-len(hash_raw) % 4))
        actual = hashlib.pbkdf2_hmac("sha256", password.encode("utf-8"), salt, 600000, dklen=32)
...
async def security_and_rate_limit_middleware(request: Request, call_next):
    ...
    if request.method in WRITE_METHODS and not is_bot and not is_public_path and path not in {"/admin/login", "/admin/logout"}:
...
@app.get("/admin/session", status_code=204)
async def admin_session(request: Request):
    if not admin_origin_allowed(request):
        raise HTTPException(status_code=403, detail="Invalid origin.")
    if not verify_admin_session(request.cookies.get(ADMIN_COOKIE_NAME)):
        raise HTTPException(status_code=401, detail="Not authenticated.")
    return Response(status_code=204, headers={"Cache-Control": "no-store"})

@app.post("/admin/login", status_code=204)
async def admin_login(request: Request):
    if not admin_origin_allowed(request):
        raise HTTPException(status_code=403, detail="Invalid origin.")
    client_ip = get_client_ip(request)
    if not rate_limit_check(client_ip, "admin-login", ADMIN_LOGIN_LIMIT, ADMIN_LOGIN_WINDOW):
        raise HTTPException(status_code=429, detail="Too many login attempts.", headers={"Retry-After": str(ADMIN_LOGIN_WINDOW)})
    try:
        payload = await request.json()
        password = payload.get("password", "") if isinstance(payload, dict) else ""
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid request body.")
    if not await asyncio.to_thread(verify_admin_password, password):
...
async def admin_logout(request: Request):
    if not admin_origin_allowed(request):
        raise HTTPException(status_code=403, detail="Invalid origin.")
    response = Response(status_code=204, headers={"Cache-Control": "no-store"})
    response.delete_cookie(ADMIN_COOKIE_NAME, path="/", domain=".ymkw.top", secure=True, httponly=True, samesite="lax")
    return response
